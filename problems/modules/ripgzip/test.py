...
def decompress_file_ripgzip(data, debug=False):
    ...
    proc = subprocess.run([path, "-d", "-v"], input=data, capture_output=True, check=True)
    return proc.stdout
...
def test_static_cases():
    for file_path in sorted(OK_TESTS_PATH.iterdir()):
        ...
        with open("expected.txt", "wb") as binary_file:
            binary_file.write(expected)
        with open("got.txt", "wb") as binary_file_s:
            binary_file_s.write(got)
...
def test_big_random_cases():
    ...
    for i in range(10):
        ...
        data = bytes(random.randrange(256) for _ in range(1000)) * 200000
        compressed = gzip.compress(data)
